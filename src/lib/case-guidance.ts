...
export type GuidanceStep = {
  ...
  // (Documents, the evidence checklist, CaseProgress's own forms) — a step
  // always points at real, already-working UI, never a new surface.
...
function evidenceSteps(evidenceItems: EvidenceItemRecord[] | undefined): GuidanceStep[] {
  if (!evidenceItems || evidenceItems.length === 0) return [];
  const missing = evidenceItems.filter((i) => i.documents.length === 0);
  if (missing.length === 0) return [];
  return [
    {
      label: `Upload evidence for ${missing.length} item${missing.length === 1 ? "" : "s"}`,
      detail: `Your evidence checklist has ${missing.length} of ${evidenceItems.length} item${evidenceItems.length === 1 ? "" : "s"} still needing a document. Stronger evidence supports a stronger case.`,
      action: { label: "Go to Evidence Checklist", anchor: "case-evidence-checklist" },
    },
  ];
}
