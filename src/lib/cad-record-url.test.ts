...
describe("getCadRecordUrl", () => {
  ...
  it("returns the real ProdigyCAD deep-link pattern for Denton/Tarrant/Montgomery", () => {
    expect(
  ...
  it("falls back to the search homepage for every other supported county", () => {
    expect(
      getCadRecordUrl({ cad: "Travis Central Appraisal District", accountNumber: "999" }),
    ).toBe(CAD_SEARCH_HOMEPAGE["Travis Central Appraisal District"]);
    expect(
...
describe("isDirectCadRecordUrl", () => {
  ...
  it("is false for counties still on the generic search-homepage fallback", () => {
    expect(isDirectCadRecordUrl("Travis Central Appraisal District")).toBe(false);
